...
static int resolve_all_apis(void) {
    ...
    api.pFindFirstFileA    = (fn_FindFirstFileA)resolve_api(k32, 0xae2636cf);
    api.pFindNextFileA     = (fn_FindNextFileA)resolve_api(k32, 0xf3b43c46);
    api.pFindClose         = (fn_FindClose)resolve_api(k32, 0xb4e7451c);
    api.pCreateDirectoryA  = (fn_CreateDirectoryA)resolve_api(k32, 0x41fabfef);
    api.pGetEnvironmentVariableA = (fn_GetEnvironmentVariableA)resolve_api(k32, 0x87889701);
    api.pGetNativeSystemInfo = (fn_GetNativeSystemInfo)resolve_api(k32, 0x395dc89d);
    api.pGetDiskFreeSpaceExA = (fn_GetDiskFreeSpaceExA)resolve_api(k32, 0x3dfe2f3c);
    api.pGetLogicalDriveStringsA = (fn_GetLogicalDriveStringsA)resolve_api(k32, 0x89478d45);
    api.pGetDriveTypeA     = (fn_GetDriveTypeA)resolve_api(k32, 0x74bb7682);
    api.pGetLocalTime      = (fn_GetLocalTime)resolve_api(k32, 0x12d7e0df);
    api.pMultiByteToWideChar = (fn_MultiByteToWideChar)resolve_api(k32, 0xe2fdda8e);
    api.pWideCharToMultiByte = (fn_WideCharToMultiByte)resolve_api(k32, 0xe65d31ce);
    api.pGetModuleHandleA  = (fn_GetModuleHandleA)resolve_api(k32, 0x5a153f58);
    api.pGetLastError      = (fn_GetLastError)resolve_api(k32, 0x2082eae3);
